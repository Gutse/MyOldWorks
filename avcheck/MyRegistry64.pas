...
type
  ...
  TRegistry64IniFile = class(TCustomIniFile)
  private
...
constructor TRegistry64.Create;
begin
...
constructor TRegistry64.Create(AAccess: LongWord);
begin
  Create;
  FAccess := AAccess;
end;

destructor TRegistry64.Destroy;
begin
  CloseKey;
  inherited;
end;

procedure TRegistry64.CloseKey;
begin
...
procedure TRegistry64.SetRootKey(Value: HKEY);
begin
...
procedure TRegistry64.ChangeKey(Value: HKey; const Path: string);
begin
  CloseKey;
  FCurrentKey := Value;
  FCurrentPath := Path;
...
function TRegistry64.GetBaseKey(Relative: Boolean): HKey;
begin
...
procedure TRegistry64.SetCurrentKey(Value: HKEY);
begin
  FCurrentKey := Value;
end;

function TRegistry64.CreateKey(const Key: string): Boolean;
var
  TempKey: HKey;
  S: string;
  Disposition: Integer;
...
function TRegistry64.OpenKey(const Key: string; Cancreate: boolean): Boolean;
var
  TempKey: HKey;
  S: string;
  Disposition: Integer;
...
function TRegistry64.OpenKeyReadOnly(const Key: string): Boolean;
var
  TempKey: HKey;
  S: string;
  Relative: Boolean;
begin
  ...
  TempKey := 0;
  Result := RegOpenKeyEx(GetBaseKey(Relative), PChar(S), 0,
    KEY_READ or KEY_WOW64_64KEY, TempKey) = ERROR_SUCCESS;
  if Result then
  begin
    FAccess := KEY_READ or KEY_WOW64_64KEY;
    if (CurrentKey <> 0) and Relative then S := CurrentPath + '\' + S;
    ChangeKey(TempKey, S);
  end
  else
  begin
    Result := RegOpenKeyEx(GetBaseKey(Relative), PChar(S), 0,
      STANDARD_RIGHTS_READ or KEY_QUERY_VALUE or KEY_ENUMERATE_SUB_KEYS or KEY_WOW64_64KEY,
      TempKey) = ERROR_SUCCESS;
    if Result then
    begin
      FAccess := STANDARD_RIGHTS_READ or KEY_QUERY_VALUE or KEY_ENUMERATE_SUB_KEYS or KEY_WOW64_64KEY;
      if (CurrentKey <> 0) and Relative then S := CurrentPath + '\' + S;
      ChangeKey(TempKey, S);
    end
    else
    begin
      Result := RegOpenKeyEx(GetBaseKey(Relative), PChar(S), 0,
        KEY_QUERY_VALUE or KEY_WOW64_64KEY, TempKey) = ERROR_SUCCESS;
      if Result then
      begin
        FAccess := KEY_QUERY_VALUE or KEY_WOW64_64KEY;
        if (CurrentKey <> 0) and Relative then S := CurrentPath + '\' + S;
        ChangeKey(TempKey, S);
      end
    end;
...
function TRegistry64.DeleteKey(const Key: string): Boolean;
var
  Len: DWORD;
  I: Integer;
  Relative: Boolean;
...
function TRegistry64.DeleteValue(const Name: string): Boolean;
begin
  Result := RegDeleteValue(CurrentKey, PChar(Name)) = ERROR_SUCCESS;
end;

function TRegistry64.GetKeyInfo(var Value: TRegKeyInfo): Boolean;
begin
...
procedure TRegistry64.GetKeyNames(Strings: TStrings);
var
  Len: DWORD;
  I: Integer;
  Info: TRegKeyInfo;
...
procedure TRegistry64.GetValueNames(Strings: TStrings);
var
  Len: DWORD;
  I: Integer;
  Info: TRegKeyInfo;
...
function TRegistry64.GetDataInfo(const ValueName: string; var Value: TRegDataInfo): Boolean;
var
  DataType: Integer;
begin
...
function TRegistry64.GetDataSize(const ValueName: string): Integer;
var
  Info: TRegDataInfo;
begin
...
function TRegistry64.GetDataType(const ValueName: string): TRegDataType;
var
  Info: TRegDataInfo;
begin
  if GetDataInfo(ValueName, Info) then
...
procedure TRegistry64.WriteString(const Name, Value: string);
begin
  PutData(Name, PChar(Value), Length(Value) + 1, rdString);
end;

procedure TRegistry64.WriteExpandString(const Name, Value: string);
begin
  PutData(Name, PChar(Value), Length(Value) + 1, rdExpandString);
end;

function TRegistry64.ReadString(const Name: string): string;
var
  Len: Integer;
  RegData: TRegDataType;
begin
...
procedure TRegistry64.WriteInteger(const Name: string; Value: Integer);
begin
  PutData(Name, @Value, SizeOf(Integer), rdInteger);
end;

function TRegistry64.ReadInteger(const Name: string): Integer;
var
  RegData: TRegDataType;
begin
...
procedure TRegistry64.WriteBool(const Name: string; Value: Boolean);
begin
  WriteInteger(Name, Ord(Value));
end;

function TRegistry64.ReadBool(const Name: string): Boolean;
begin
  Result := ReadInteger(Name) <> 0;
end;

procedure TRegistry64.WriteFloat(const Name: string; Value: Double);
begin
  PutData(Name, @Value, SizeOf(Double), rdBinary);
end;

function TRegistry64.ReadFloat(const Name: string): Double;
var
  Len: Integer;
  RegData: TRegDataType;
begin
...
procedure TRegistry64.WriteCurrency(const Name: string; Value: Currency);
begin
  PutData(Name, @Value, SizeOf(Currency), rdBinary);
end;

function TRegistry64.ReadCurrency(const Name: string): Currency;
var
  Len: Integer;
  RegData: TRegDataType;
begin
...
procedure TRegistry64.WriteDateTime(const Name: string; Value: TDateTime);
begin
  PutData(Name, @Value, SizeOf(TDateTime), rdBinary);
end;

function TRegistry64.ReadDateTime(const Name: string): TDateTime;
var
  Len: Integer;
  RegData: TRegDataType;
begin
...
procedure TRegistry64.WriteDate(const Name: string; Value: TDateTime);
begin
  WriteDateTime(Name, Value);
end;

function TRegistry64.ReadDate(const Name: string): TDateTime;
begin
  Result := ReadDateTime(Name);
end;

procedure TRegistry64.WriteTime(const Name: string; Value: TDateTime);
begin
  WriteDateTime(Name, Value);
end;

function TRegistry64.ReadTime(const Name: string): TDateTime;
begin
  Result := ReadDateTime(Name);
end;

procedure TRegistry64.WriteBinaryData(const Name: string; var Buffer; BufSize: Integer);
begin
  PutData(Name, @Buffer, BufSize, rdBinary);
end;

function TRegistry64.ReadBinaryData(const Name: string; var Buffer; BufSize: Integer): Integer;
var
  RegData: TRegDataType;
  Info: TRegDataInfo;
begin
...
procedure TRegistry64.PutData(const Name: string; Buffer: Pointer;
...
function TRegistry64.GetData(const Name: string; Buffer: Pointer;
  BufSize: Integer; var RegData: TRegDataType): Integer;
var
  DataType: Integer;
begin
...
function TRegistry64.HasSubKeys: Boolean;
var
  Info: TRegKeyInfo;
begin
...
function TRegistry64.ValueExists(const Name: string): Boolean;
var
  Info: TRegDataInfo;
begin
...
function TRegistry64.GetKey(const Key: string): HKEY;
var
  S: string;
  Relative: Boolean;
begin
...
function TRegistry64.RegistryConnect(const UNCName: string): Boolean;
var
  TempKey: HKEY;
begin
...
function TRegistry64.LoadKey(const Key, FileName: string): Boolean;
var
  S: string;
begin
  S := Key;
...
function TRegistry64.UnLoadKey(const Key: string): Boolean;
var
  S: string;
begin
  S := Key;
...
function TRegistry64.RestoreKey(const Key, FileName: string): Boolean;
var
  RestoreKey: HKEY;
begin
  Result := False;
...
function TRegistry64.ReplaceKey(const Key, FileName, BackUpFileName: string): Boolean;
var
  S: string;
  Relative: Boolean;
begin
...
function TRegistry64.SaveKey(const Key, FileName: string): Boolean;
var
  SaveKey: HKEY;
begin
  Result := False;
...
function TRegistry64.KeyExists(const Key: string): Boolean;
var
  TempKey: HKEY;
  OldAccess: Longword;
begin
...
procedure TRegistry64.RenameValue(const OldName, NewName: string);
var
  Len: Integer;
  RegData: TRegDataType;
  Buffer: PChar;
...
procedure TRegistry64.MoveKey(const OldName, NewName: string; Delete: Boolean);
var
  SrcKey, DestKey: HKEY;
...
constructor TRegistry64IniFile.Create(const FileName: string);
begin
  Create(FileName, KEY_ALL_ACCESS);
end;

constructor TRegistry64IniFile.Create(const FileName: string; AAccess: LongWord);
begin
  inherited Create(FileName);
  FRegIniFile := TRegIniFile.Create(FileName, AAccess);
end;

destructor TRegistry64IniFile.Destroy;
begin
  FRegIniFile.Free;
  inherited Destroy;
end;

function TRegistry64IniFile.ReadString(const Section, Ident, Default: string): string;
begin
  Result := FRegIniFile.ReadString(Section, Ident, Default);
end;

function TRegistry64IniFile.ReadDate(const Section, Name: string; Default: TDateTime): TDateTime;
var
  Key, OldKey: HKEY;
begin
  with FRegIniFile do
...
function TRegistry64IniFile.ReadDateTime(const Section, Name: string; Default: TDateTime): TDateTime;
var
  Key, OldKey: HKEY;
begin
  with FRegIniFile do
...
function TRegistry64IniFile.ReadFloat(const Section, Name: string; Default: Double): Double;
var
  Key, OldKey: HKEY;
begin
  with FRegIniFile do
...
function TRegistry64IniFile.ReadInteger(const Section, Ident: string; Default: LongInt): LongInt;
var
  Key, OldKey: HKEY;
begin
  with TRegistry64(FRegIniFile) do
  begin
    Key := GetKey(Section);
    if Key <> 0 then
    try
...
function TRegistry64IniFile.ReadTime(const Section, Name: string; Default: TDateTime): TDateTime;
var
  Key, OldKey: HKEY;
begin
  with FRegIniFile do
...
function TRegistry64IniFile.ReadBinaryStream(const Section, Name: string; Value: TStream): Integer;
var
  RegData: TRegDataType;
  Info: TRegDataInfo;
  Key, OldKey: HKEY;
...
begin
  ...
  with RegIniFile do
  begin
    Key := TRegistry64(FRegIniFile).GetKey(Section);
    if Key <> 0 then
    try
      OldKey := CurrentKey;
      TRegistry64(FRegIniFile).SetCurrentKey(Key);
      try
        if ValueExists(Name) then
        begin
          if GetDataInfo(Name, Info) then
        ...
        TRegistry64(FRegIniFile).SetCurrentKey(OldKey);
      end;
    finally
      RegCloseKey(Key);
    end;
...
procedure TRegistry64IniFile.WriteDate(const Section, Name: string; Value: TDateTime);
var
  Key, OldKey: HKEY;
begin
  with FRegIniFile do
...
procedure TRegistry64IniFile.WriteDateTime(const Section, Name: string; Value: TDateTime);
var
  Key, OldKey: HKEY;
begin
  with FRegIniFile do
...
procedure TRegistry64IniFile.WriteFloat(const Section, Name: string; Value: Double);
var
  Key, OldKey: HKEY;
begin
  with FRegIniFile do
...
procedure TRegistry64IniFile.WriteInteger(const Section, Ident: string; Value: LongInt);
var
  Key, OldKey: HKEY;
begin
  with TRegistry64(FRegIniFile) do
...
procedure TRegistry64IniFile.WriteTime(const Section, Name: string; Value: TDateTime);
var
  Key, OldKey: HKEY;
begin
  with FRegIniFile do
...
procedure TRegistry64IniFile.WriteBinaryStream(const Section, Name: string;
  Value: TStream);
var
  Key, OldKey: HKEY;
  Stream: TMemoryStream;
...
procedure TRegistry64IniFile.WriteString(const Section, Ident, Value: string);
begin
  FRegIniFile.WriteString(Section, Ident, Value);
end;

procedure TRegistry64IniFile.ReadSection(const Section: string; Strings: TStrings);
begin
  FRegIniFile.ReadSection(Section, Strings);
end;

procedure TRegistry64IniFile.ReadSections(Strings: TStrings);
begin
  FRegIniFile.ReadSections(Strings);
end;

procedure TRegistry64IniFile.ReadSectionValues(const Section: string; Strings: TStrings);
begin
  FRegIniFile.ReadSectionValues(Section, Strings);
end;

procedure TRegistry64IniFile.EraseSection(const Section: string);
begin
  FRegIniFile.EraseSection(Section);
end;

procedure TRegistry64IniFile.DeleteKey(const Section, Ident: string);
begin
  FRegIniFile.DeleteKey(Section, Ident);
end;

procedure TRegistry64IniFile.UpdateFile;
begin
  { Do nothing }
end;
